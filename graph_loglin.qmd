...
data_bc <- data.frame(expand.grid(X1 = c("Boston", "Glamorgan"), X2 = c("malignant", "benign"), X3 = c("died", "survived"), stringsAsFactors = TRUE), Freq)

data_bc

ftable(X1 + X2 ~ X3, xtabs(Freq ~., data_bc))
```

Fit a saturated model 

```{r}
m_sat  <-glm(Freq ~ X1 * X2 * X3, family = poisson, data = data_bc)
m_sat
```
...
```{r}
m_ci <- glm(Freq ~ X1 * X2 + X1 * X3, family = poisson, data = data_bc)
m_ci
```
